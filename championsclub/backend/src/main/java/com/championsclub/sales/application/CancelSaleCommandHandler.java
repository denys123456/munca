...
public class CancelSaleCommandHandler {
    ...
    public SaleResponse cancel(long id) {
        var existing = sales.find(id)
                .orElseThrow(() -> new ResourceNotFoundException("SALE_NOT_FOUND", "Sale not found."));
        access.advisor(existing.advisorId());
        users.lock(existing.advisorId());
        var sale = sales.lock(id).orElseThrow();
        if (!sale.contributesToPerformance()) {
            throw new BusinessRuleViolationException("SALE_ALREADY_CANCELLED", "The sale is already cancelled.");
        }
        var cancelled = sales.save(sale.cancel());
        if (sale.awardedPoints() > 0) {
            points.append(
                    sale.advisorId(),
                    PointTransactionType.SALE_REVERSAL,
                    -sale.awardedPoints(),
                    sale.id(),
