...
for (const collection of collections) {
  test(collection.route + ' supports create, edit, search and confirmed deletion', async ({ page }) => {
    ...
    await expect(page.locator('#workspace-content .admin-row')).toHaveCount(1)
    ...
    await page.getByLabel(collection.search, { exact: true }).fill(collection.name)
    await expect(page.locator('#workspace-content .admin-row')).toContainText(collection.name + ' Plus')
    await page.getByRole('button', { name: 'Delete ' + collection.name + ' Plus', exact: true }).click()
    await page.getByRole('button', { name: 'Cancel', exact: true }).click()
    await expect(page.locator('#workspace-content .admin-row')).toHaveCount(1)
    await page.getByRole('button', { name: 'Delete ' + collection.name + ' Plus', exact: true }).click()
    await page.getByRole('button', { name: 'Delete entry', exact: true }).click()
    await expect(page.locator('#workspace-content .admin-row')).toHaveCount(0)
    await expect(page.locator('#workspace-content .empty-state')).toContainText('No results')
    await navigate(page, 'admin-audit')
    await expect(page.locator('#workspace-content .audit-timeline article')).toHaveCount(3)
  })
}
...
test('membership thresholds update all affected advisors and persist', async ({ page }) => {
  ...
  await expect(page.locator('#workspace-content .membership-level')).toContainText('GOLD')
