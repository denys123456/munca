...
export function PerformancePage({ data, role }) {
  ...
  const predicted = data.forecast.predictedSales * (data.forecastSource === 'live' && data.forecastScope === 'advisor' ? 1 : factor)
  ...
  return <div className="page-stack performance-page">
    ...
    <section className="kpi-grid"><KpiCard icon={BadgeEuro} label="Sales booked" value={formatCurrency(sales)} detail="Current cycle volume" /><KpiCard icon={Target} label="Target achievement" value={`${progress}%`} detail="Actual versus your target" tone="success" /><KpiCard icon={TrendingUp} label="Projected finish" value={formatCurrency(predicted)} detail={data.forecastSource === 'live' ? 'Service forecast' : 'Illustrative demo forecast'} /><KpiCard icon={BarChart3} label="Sustainable sales" value={`${data.dealership.sustainableSalesShare}%`} detail="Dealership green mobility mix" /></section>
    <section className="performance-analysis"><article className="performance-canvas"><SectionHeader eyebrow="ACTUAL. PROJECTED. POSSIBLE." title="The shape of your performance" /><ForecastChart actual={sales} predicted={predicted} target={target} history={data.charts.monthlySales.map((value) => value * factor)} /><p className="soft-copy chart-disclosure">Historical context uses the included demonstration series. Forecast estimates are indicative.</p></article>
